function unweightedSearchAlgorithm(nodes, start, target, nodesToAnimate, boardArray, name) {
  ...
  let exploredNodes = {start: true};  //treat explored nodes as a Hashmap
...
function getNeighbors(id, nodes, boardArray, name) {
  ...
  if (boardArray[x - 1] && boardArray[x - 1][y])         //upper - neighbor
  { 
  ...
  if (boardArray[x][y + 1]) {                            // right - neighbor
  ...
  if (boardArray[x + 1] && boardArray[x + 1][y]) {       // down - neighbor
    potentialNeighbor = `${(x + 1).toString()}-${y.toString()}`
    if (nodes[potentialNeighbor].status !== "wall") {
      if (name === "bfs") {
        neighbors.push(potentialNeighbor);
  ...
  if (boardArray[x][y - 1]) {                             //  left - neighbor
    potentialNeighbor = `${x.toString()}-${(y - 1).toString()}`
    if (nodes[potentialNeighbor].status !== "wall") {
      if (name === "bfs") {
        neighbors.push(potentialNeighbor);
