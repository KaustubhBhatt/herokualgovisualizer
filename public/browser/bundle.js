...
Board.prototype.toggleTutorialButtons = function() {
  ...
  function nextPreviousClick() {
    if (counter === 1) {
      ...
      document.getElementById("tutorial").innerHTML = `<h3>Adding walls</h3><h6>Click on the grid to add a wall. Click on the grid while pressing W to add a weight. Generate mazes and patterns from the "Mazes & Patterns" drop-down menu.</h6><p>Walls are impenetrable, meaning that a path cannot cross through them. Weights, however, are not impassable. They are simply more "costly" to move through. In this application, moving through a weight node has a "cost" of 15.</p><img id="secondTutorialImage" src="public/styling/walls.gif"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`
    } else if (counter === 6) {
      document.getElementById("tutorial").innerHTML = `<h3>Adding a Flag</h3><h6>Click the "Add Flag" button.</h6><p>Adding a Flag will change the course of the chosen algorithm. In other words, the algorithm will first look for the flag (seen as a checkpoint) and will then look for the finish line. Note that the Bidirectional Swarm Algorithm does not support adding a Flag.</p><img id="secondTutorialImage" src="public/styling/add_flag.png"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`
    } else if (counter === 7) {
...
Board.prototype.toggleButtons = function() {
  ...
  if (!this.buttonsOn) {
    ...
    document.getElementById("startButtonClearBoard").onclick = () => {
      document.getElementById("startButtonAddObject").innerHTML = '<a href="#">Add Flag</a></li>';
      ...
      if (this.currentAlgorithm !== "bidirectional") {
        if (innerHTML.includes("Add")) {
          ...
          if (this.target === objectNodeId || this.start === objectNodeId || this.numberOfObjects === 1) {
            ...
            document.getElementById("startButtonAddObject").innerHTML = '<a href="#">Remove Flag</a></li>';
            this.clearPath("clickedButton");
